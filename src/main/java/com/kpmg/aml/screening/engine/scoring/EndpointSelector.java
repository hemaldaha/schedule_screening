...
public class EndpointSelector {
    ...
    private volatile Runnable criticalAlertHook;
    ...
    void registerCriticalAlertHook(Runnable hook) {
        this.criticalAlertHook = hook;
    }
        ...
        do {
            ...
            if (selectedUrl == null) {
                ...
                if (attemptCount >= maxAttempts) {
                    ...
                    if (criticalAlertHook != null) {
                        Thread.ofVirtual().start(criticalAlertHook);
                    }
                    break;
                }
    ...
    void resetHealth() {
        ...
        for (String endpoint : endpoints) {
            ...
            try {
                ...
                boolean wasHealthy = healthMap.getOrDefault(endpoint, true);
                healthMap.put(endpoint, isHealthy);

                if (!isHealthy) {
                    log.warn("[EndpointSelector] Health check DOWN: {}", endpoint);
                } else if (!wasHealthy) {
                    log.info("[EndpointSelector] Health check RECOVERED: {}", endpoint);
                }

            } catch (IOException | InterruptedException ex) {
                healthMap.put(endpoint, false);
                log.warn("[EndpointSelector] Health check unreachable {}: {}", endpoint, ex.getMessage());
            }

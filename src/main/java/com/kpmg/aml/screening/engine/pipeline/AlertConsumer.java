...
public class AlertConsumer {
    ...
    public AlertConsumer(ScreeningGovernor governor,
            ...
            @Lazy DeadLetterService deadLetterService) {
    ...
    private void drain() {
        ...
        while (true) {
            ...
            metrics.updateAlertBufferDepth(payloadBatch.size());
    ...
    private void persistBatch(List<AlertPayload> batch) {
        ...
        try {
            ...
            metrics.recordAlertsPersisted(saved.size());

        } catch (Exception ex) {
            log.error("[AlertConsumer] Batch persist failed — forwarding {} records to DeadLetterService. reason={}",
                    batch.size(), ex.getMessage(), ex);
            batch.forEach(deadLetterService::handleMissedAlert);
        }
    }
    ...
    AlertEntity buildAlertEntity(AlertPayload payload,
        ...
        entity.setStatus(AlertStatus.unassigned);
